...
class MyInterface:
    ...
    #         else:
    #             runtime = int(runtime)
    #             total_runtime = time.time() + runtime
    ...
    #     # Continuously read data from anemometers and log it
    #     while self.logging_active and self.master.winfo_exists():
    #         row_data = []  # Initialize row data outside of the loop
    #         current_time = time.time()
    #         elapsed_time = current_time - last_log_time

    #         # Update last log time every second
    #         if elapsed_time >= 0.2:
    ...
    #                 if elapsed_time >= 0.2:
    #                     time_sec += 0.2
    #                     # row_data = [str(self.update_stopwatch(start_time))]
    #                     row_data = [time.strftime("%Y-%m-%d %H:%M:%S")]
    #                     row_data.append(str(time_sec))
    #                     row_data.append(str(pedal_rpm))
    #                     row_data.append(self.power_conversion(str(pedal_rpm)))
    #                     row_data.append(str(smoothed_rpm_value))
    #                     row_data.append(str(smoothed_power_value))
    ...
    def update_gauges(self, rpm):
        ...
        # print(f"Smoothed RPM: {smoothed_value} and Raw RPM: {rpm}")

        # self.rpm_gauge.set(round(float(rpm),1))
        # self.rpm_gauge.update()
        # self.power_gauge.set(self.power_conversion(rpm))
        # self.power_gauge.update()
        

#     def power_conversion(self, rpm):
#         power = 0.0014 * float(rpm) ** 3 - 0.0223 * float(rpm) ** 2 + 2.072 * float(rpm) - 8.0112
#         power = round(power, 1)
#         return power
...
#         return 0.0014 * rpm**3 - 0.0223 * rpm**2 + 2.072 * rpm - 8.0112 - P
